...
<head>
    ...
    <body>
        ...
        if ($result->num_rows > 0) {
            echo "<h2>Kõik albumid:</h2>";
            while($row = $result->fetch_assoc()) {
                echo "<p>";
                echo "Artist: " . $row["artist"] . " - Album: " . $row["album"] . " - Aasta: " . $row["aasta"] . " - Hind: " . $row["hind"] . "€";
                echo " <a href='?action=muuda&id=" . $row["id"] . "'>Muuda</a>";
                echo " <a href='?action=kustuta&id=" . $row["id"] . "' onclick=\"return confirm('STOP SEIS! Kas oled Päriselt kindel?');\">Kustuta</a>";
                echo "</p>";
            }
        }
        ?>
        ...
        <?php
        $sql = "SELECT kliendid.eesnimi, kliendid.perenimi, arved.arve_nr, albumid.album FROM kliendid INNER JOIN arved ON arved.kliendid_id = kliendid.id INNER JOIN albumid ON arved.albumid_id = albumid.id;";
        $result = $yhendus->query($sql);
